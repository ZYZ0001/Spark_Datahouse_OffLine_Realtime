...
object QzLogProducer {
  def main(args: Array[String]): Unit = {
    val conf: SparkConf = new SparkConf().setAppName("QzLogProducer").setMaster("local[*]")
    val ssc = new SparkContext(conf)
    ssc.textFile("E:\\BigDate\\39.在线教育项目\\2.资料\\日志数据\\qz.log", 10)
      .foreachPartition(par => {
        val properties: Properties = MyPropertiesUtil.getProperties("kafka_producer.properties")
        val producer = new KafkaProducer[String, String](properties)
        par.foreach(item => {
          val request = new ProducerRecord[String, String](RealtimeConstant.KAFKA_QZ_LOG_TOPIC, item)
